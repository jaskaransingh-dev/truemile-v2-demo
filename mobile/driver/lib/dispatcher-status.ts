...
export function statusLabel(s?: string | null) { return s ? (STATUS_MAP[s]?.label || s) : 'Unknown' }
export function statusColor(s?: string | null) { return s ? (STATUS_MAP[s]?.color || '#6B7280') : '#6B7280' }
export function statusPinColor(s?: string | null) { return s ? (STATUS_MAP[s]?.pinColor || 'gray') : 'gray' }
